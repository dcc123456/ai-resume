// resume-ai/server/src/services/generateService.js
const db = require('../db/connection');
const { getLLM } = require('../llm');
const { SYSTEM_PROMPT, buildGeneratePrompt, RETRY_APPEND } = require('../prompts/resumeGenerate');
const { factCheck } = require('./factCheckService');
...
const { generateDocxBuffer } = require('../utils/docxGenerator');
const { saveFile } = require('./fileService');
const { marked } = require('marked');
const puppeteer = require('puppeteer');

async function generateCustomResume(userId, jdId) {
  const jds = await db.query('SELECT * FROM job_description WHERE id=? AND user_id=?', [jdId, userId]);
  if (jds.length === 0) throw new Error('JD不存在');
  const bases = await db.query('SELECT * FROM base_resume WHERE user_id=?', [userId]);
  ...
  const result = await db.query('INSERT INTO custom_resume (user_id, jd_id, generated_json, markdown_text, fact_check_report, keyword_match_rate) VALUES (?,?,?,?,?,?)', [userId, jdId, JSON.stringify(generatedJson), markdown, JSON.stringify(factReport), matchRate]);
  return { resume_id: result.insertId, generated_json: generatedJson, markdown, fact_check_report: factReport, keyword_match_rate: matchRate, missing_keywords: missingKeywords };
}

async function updateResumeMarkdown(userId, resumeId, markdownText) {
  await db.query('UPDATE custom_resume SET markdown_text=? WHERE id=? AND user_id=?', [markdownText, resumeId, userId]);
  return { success: true };
}

async function downloadResume(userId, resumeId, format) {
  const resumes = await db.query('SELECT * FROM custom_resume WHERE id=? AND user_id=?', [resumeId, userId]);
  ...
  if (format === 'docx') {
    ...
    await db.query('UPDATE custom_resume SET download_url_docx=? WHERE id=?', [filePath, resumeId]);
  ...
  if (format === 'pdf') {
    ...
    await db.query('UPDATE custom_resume SET download_url_pdf=? WHERE id=?', [filePath, resumeId]);
    return { filename: `resume_${resumeId}.pdf`, buffer };
  }
  throw new Error('不支持的格式');
}

async function getGenerateHistory(userId, jdId = null) {
  ...
  return { items: rows.map((r) => ({ resume_id: r.id, jd_title: r.parsed_json?.title || '未命名', created_at: r.created_at, match_rate: r.keyword_match_rate })) };
}

async function deleteCustomResume(userId, resumeId) {
  const result = await db.query('DELETE FROM custom_resume WHERE id=? AND user_id=?', [resumeId, userId]);
  return { success: result.affectedRows > 0 };
}

module.exports = { generateCustomResume, updateResumeMarkdown, downloadResume, getGenerateHistory, deleteCustomResume };
