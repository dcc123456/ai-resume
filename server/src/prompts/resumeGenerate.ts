// src/prompts/resumeGenerate.ts
export const SYSTEM_PROMPT = '你是一位资深简历顾问。你的任务是根据职位描述和基础简历，生成一份高度匹配的定制化简历。严格遵守事实，绝不编造经历。';

const USER_TEMPLATE = `根据【职位描述】和【基础简历】生成定制化简历JSON，规则：
1. 禁止修改公司名称、职位名称、起止时间——这些是事实信息，必须与基础简历完全一致。
2. 重写工作职责描述，自然融入JD中的硬技能关键词，使用STAR法则。
